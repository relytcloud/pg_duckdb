...
def test_autoinstall_known_extensions(pg: Postgres, cur: Cursor):
    ...
    assert (
        cur.sql(
            "SELECT * FROM duckdb.query($$ FROM duckdb_extensions() SELECT installed, loaded WHERE extension_name = 'lance' $$)"
        )
        == []
    )

    cur.sql("SELECT duckdb.install_extension('lance', 'community')")
    assert cur.sql(
        "SELECT * FROM duckdb.query($$ FROM duckdb_extensions() SELECT installed, loaded WHERE extension_name = 'lance' $$)"
    ) == (True, True)
    ...
    with pytest.raises(
        ...
        match=r'lance.duckdb_extension" not found.',
    ):
