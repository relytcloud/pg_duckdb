...
def test_community_extensions(pg: Postgres):
    ...
    with pg.cur() as cur:
        ...
        with pytest.raises(
            ...
            cur.sql(
                "SELECT * FROM duckdb.raw_query($$ INSTALL lance FROM community; $$)"
            )
    ...
    with pg.cur() as cur:
        cur.sql("SET duckdb.force_execution = false")
        cur.sql("SELECT * FROM duckdb.raw_query($$ INSTALL lance FROM community; $$)")
        with pytest.raises(
            Exception,
            ...
            cur.sql("SELECT * FROM duckdb.raw_query($$ LOAD lance; $$)")
